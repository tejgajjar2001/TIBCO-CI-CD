...
if [ $# != 4 ]
    then
        echo "USAGE:\ <uDeploy_WorkingDir> <DOMAIN_NAME> <APP_SECURITY.cred> <START.batch>"
        exit 1
fi
...
$scriptloc/batchstart.sh ${UD_DIR} ${DOM_NAME} ${master_cred} ${4} >> $workingdir/logs/scripted_deploy.log
   if [ $? -eq 0 ]
        then
        echo -e "BatchStart completed successfully."
        ...
        echo -e "there were somthing wrong while batchstart, please investigate."
        cat $workingdir/logs/scripted_deploy.log >> $workingdir/logs/batchexecution.log
        echo -e " -------------------------------------------------------- " >> $workingdir/logs/batchexecution.log
        echo -e " -------------------------------------------------------- " >> $workingdir/logs/appmgmt.log.archive
        echo -e "INFO:\t $0 Application Management log details." >> $workingdir/logs/appmgmt.log.archive
