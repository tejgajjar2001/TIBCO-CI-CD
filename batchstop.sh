...
if [ -z "$applist_name" ] || [ $applist_name == "AppManage.batch" ]; then
   ...
   if [ -e "$applist_loc/AppManage.batch" ]; then
      ...
      echo -e "$applist_loc/AppManage.batch found. Proceed to Stop."
  fi
...
echo -e "INFO:\t $exec_dt : \tBATCH DEPLOY (BatchStop): $domain_name - artifacts: $applist_loc"
printAppList

# Execute the AppManage command
#cd $appmanagedir
cd $TRA_HOME/bin
eval ./AppManage_${domain_name} -batchStop -domain ${domain_name} -cred \"${cred_file}\" -dir \"${applist_loc}\" -serialize >> "${applist_loc}/${exec_dt}.batchStop.log"

cd ${applist_loc}
cat ${applist_loc}/${exec_dt}.batchStop.log >> ${workingdir}/logs/batchexecution.log
mv ${applist_loc}/${exec_dt}.batchStop.log ${workingdir}/logs/
...
exec_dt=`date +"%Y%m%d%H%M%S"`
echo -e "INFO:\t ${exec_dt} : \tBatchStop CMD EXECUTED:  $domain_name "
