...
# Author: Tejas Gajjar
# ===========================================================
# Executes the deployment framwork option to batchUndeploy
# the entire repository
# Syntax:
#     AppManage -batchUndeploy -domain $1 -cred $2 -dir $3 -serialize
...
if [ -z "$applist_name" ] || [ $applist_name == "AppManage.batch" ]; then
   ...
   if [ -e "$applist_loc/AppManage.batch" ]; then
      ...
      echo -e "$applist_loc/AppManage.batch found.Proceed to undeploy."
  fi
...
echo -e "INFO:\t $exec_dt : \tBATCH UNDEPLOY (BatchUnDeploy): $domain_name - artifacts: $applist_loc"
printAppList

# Execute the AppManage command
#cd $appmanagedir
cd $TRA_HOME/bin
eval ./AppManage_${domain_name} -batchUndeploy -domain ${domain_name} -cred \"${cred_file}\" -dir \"${applist_loc}\" -serialize -force >> "${applist_loc}/${exec_dt}.batchUndeploy.log"

cd ${applist_loc}
cat ${applist_loc}/${exec_dt}.batchUndeploy.log >> ${workingdir}/logs/batchexecution.log
mv ${applist_loc}/${exec_dt}.batchUndeploy.log ${workingdir}/logs/
#cd $DF_HOME/scripts
cd $currdir
exec_dt=`date +"%Y%m%d%H%M%S"`
echo -e "INFO:\t ${exec_dt} : \tBatchUnDeploy CMD EXECUTED:  $domain_name "
