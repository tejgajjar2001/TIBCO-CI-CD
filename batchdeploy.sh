...
# Syntax:
#     AppManage -batchDeploy -domain $1 -cred $2 -dir $3 -nostart -serialize
...
if [ -z "$applist_name" ] || [ $applist_name == "AppManage.batch" ]; then
   ...
   if [ -e "$applist_loc/AppManage.batch" ]; then
      ...
      mv $applist_loc/AppManage.batch $applist_loc/AppManage.batch.$exec_dt
      #cp $applist_loc/AppManage.batch $applist_loc/AppManage.batch.$exec_dt
      echo -e "$applist_loc/AppManage.batch file found. Proceed to deploy."
  fi
...
echo -e "INFO:\t $exec_dt : \tBATCH DEPLOY (BatchDeploy): $domain_name - artifacts: $applist_loc"
printAppList

# Execute the AppManage command
#cd $appmanagedir
cd $TRA_HOME/bin
# Export all existing apps - done at anttibco level
# eval ./AppManage -batchDeploy -domain ${domain_name} -cred \"${cred_file}\" -dir \"${export_dir}\"

eval ./AppManage_${domain_name} -batchDeploy -domain ${domain_name} -cred \"${cred_file}\" -dir \"${applist_loc}\" -nostart -serialize -force >> "${applist_loc}/${exec_dt}.batchDeploy.log"

cd ${applist_loc}
cat ${applist_loc}/${exec_dt}.batchDeploy.log >> ${workingdir}/logs/batchexecution.log
mv ${applist_loc}/${exec_dt}.batchDeploy.log ${workingdir}/logs/
cd $currdir

exec_dt=`date +"%Y%m%d%H%M%S"`
echo -e "INFO:\t ${exec_dt} : \tBatchDeploy CMD EXECUTED:  $domain_name "
