...
if [ $# != 4 ]
    then
        echo "USAGE:\ <COMP_NAME> <DOMAIN_NAME> <APP_SECURITY.cred> <DEPLOY.batch>"
        exit 1
fi
...
adminlog="$TIB_HOME/tra/domain/${DOM_NAME}/logs/ApplicationManagement.log"
        sleep 10
        echo " script is now executing."
        echo -e "$scriptloc/batchdeploy.sh ${UD_DIR} ${DOM_NAME} ${master_cred} ${4}"
        $scriptloc/batchdeploy.sh ${UD_DIR} ${DOM_NAME} ${master_cred} ${4} >> $workingdir/logs/scripted_deploy.log
   if [ $? -eq 0 ]
        then
        echo -e "BatchDeploy completed successfully."
        ...
        echo -e "there were somthing wrong while batchdeploy, please investigate."
        cat $workingdir/logs/scripted_deploy.log >> $workingdir/logs/batchexecution.log
        echo -e " -------------------------------------------------------- " >> $workingdir/logs/batchexecution.log
        echo -e " -------------------------------------------------------- " >> $workingdir/logs/appmgmt.log.archive
        echo -e "INFO:\t $0 Application Management log details." >> $workingdir/logs/appmgmt.log.archive
